...
st.write("Project by K. Janani, BTech CSE Hons, KL University [Github link](https://github.com/k-Janani/MovieMatch)")
st.title("Movie Match")
def load_lottiefile(filepath: str): #Adding Animations
...
usernames = ['kamakshi1','jsmith','rbriggs','janani'] #Data for login
...
def recommend(movie): #Function for recommending movie
...
def random_emoji():#Adding a emoji in button
...
audio_bytes = audio_file.read() #Adding audio file
...
movie_time = st.slider( #Allows you to render a range slider by passing a two-element tuple or list as the value.
     "2. Your Movie time?",
...
chart_data = pd.DataFrame( #Plotting data over area chart
     np.random.randn(50, 3),
     columns=['Comedy', 'Musical', 'Thriller'])

st.area_chart(chart_data) #Implementing Select box and radio button
